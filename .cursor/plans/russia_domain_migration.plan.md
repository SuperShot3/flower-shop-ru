...
overview: "Launch a Russia-accessible flower storefront: fork codebase into a new project, register domain + Timeweb VPS (Ubuntu 22.04), run Next.js + PostgreSQL + catalog images on one server (no Yandex Cloud), migrate catalog from Lanna Bloom Supabase, add Yandex Metrica + RU payment APIs, Russian-first localization with legal pages, then test from Russian IPs."
todos:
  - id: step0-isolation
  ...
  - id: step1-fork-codebase
    ...
    status: pending
  - id: step2-domain
    content: "Step 2 — Register new .ru domain (you); A record → Timeweb VPS IP"
    status: pending
  - id: step3-vps
    content: "Step 3 — Timeweb VPS: Ubuntu 22.04, Cloud 50 (4 GB); Docker Compose (Postgres + app); nginx + certbot"
    status: pending
  - id: step4-migrate-images-data
    content: "Step 5 — Mirror catalog images Supabase → VPS disk; import catalog rows into VPS Postgres with /catalog/ URLs"
    status: pending
  - id: step5-vps-deploy
    content: "Step 4 — Deploy app on VPS; rsync images to /var/www/catalog/"
    status: pending
...
flowchart LR
  ...
  ReadExport -->|"download images + catalog rows"| VPSRU[Timeweb VPS]
...
|--------------------------------------|----------------------------|
| `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` | Delete `lib/supabase/*`; replace with `lib/db/*` → VPS Postgres. **Do not** leave fallbacks to Supabase. |
...
| `BLOB_READ_WRITE_TOKEN` | Remove Vercel Blob uploads; use VPS disk (`CATALOG_STORAGE_DIR`). |
...
| `AUTH_SECRET` + Thailand `admin_users` | New `AUTH_SECRET`; new admin seed in VPS Postgres — **no** shared admin session with Thailand. |
...
| New env var | Purpose |
|-------------|---------|
| `DATABASE_URL` | Postgres on VPS (Docker Compose service `postgres`) |
| `POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB` | Docker Compose Postgres bootstrap |
| `CATALOG_STORAGE_DIR` | `/var/www/catalog` on VPS (image files) |
| `CATALOG_PUBLIC_BASE_URL` | `https://yourdomain.ru/catalog` (no CDN for MVP) |
...
flowchart TB
  ...
  subgraph russia [Russia — new project]
    ...
    VPS["Timeweb VPS — Ubuntu 22.04"]
    PG["PostgreSQL Docker"]
    Disk["/var/www/catalog images"]
  ...
  SupabaseTH -.->|"one-time export"| Disk
  SupabaseTH -.->|"one-time import"| PG
```
...
|------|------------------|--------------|
| **Hosts Next.js app** | Vercel serverless | **Timeweb VPS** (Docker) behind nginx |
| **Database** | Supabase Postgres | **PostgreSQL Docker** on same VPS |
| **Image files** | Supabase Storage `catalog` bucket | **VPS disk** `/var/www/catalog/` (nginx `/catalog/*`) |
| **DNS** | Porkbun → Vercel | Registrar → VPS **A record** only (no CDN for MVP) |
| **Monthly infra** | ~$45 (Vercel + Supabase) | **~1 000–1 200 ₽** (~$10–13) one VPS |
...
1. **Files** → download to `data/catalog/`, rsync to VPS `/var/www/catalog/bouquets/...`
2. **Metadata** → import catalog tables into VPS Postgres with `public_url` pointing to `https://yourdomain.ru/catalog/...`
...
3. **Replace data layer** — `lib/supabase/*` → `lib/db/*` (VPS Postgres via `pg`)
...
8. **Config** — `next.config.js`: `images.remotePatterns` → `yourdomain.ru` only
...
- DNS records (after VPS is created):

| Record | Points to |
|--------|-----------|
| `A` `@` | Timeweb VPS public IP |
| `A` `www` | Timeweb VPS public IP (or redirect www → apex) |

- SSL: Let's Encrypt on VPS (certbot) — one certificate for the main domain

---

## Step 3 — Timeweb VPS (replaces Vercel + Yandex Cloud)

Order at [Timeweb Cloud](https://timeweb.cloud):

| Setting | Choice |
|---------|--------|
| OS | **Ubuntu 22.04 LTS** |
| Plan | **Cloud 50** — 4 GB RAM, 50 GB SSD (~1 180 ₽/mo) |
| Region | Russian DC closest to customers (MSK / SPb / Novosibirsk) |

**Stack on VPS (all on one machine):**

```
nginx (443) → Next.js :3000 (Docker)
           → /catalog/* static files from /var/www/catalog/
PostgreSQL (Docker, localhost:5432)
certbot (SSL)
```

Use [`docker-compose.yml`](docker-compose.yml) — Postgres + app containers.

Apply schema: [`db/migrations/001_catalog_schema.sql`](db/migrations/001_catalog_schema.sql).
...
- PostgreSQL (Docker)
- Catalog image files on disk
- API routes, cron jobs (replace `vercel.json`)
...
```
scripts/mirror-catalog-to-vps.ts
  ...
  3. Download each file to data/catalog/
  4. rsync data/catalog/ → VPS /var/www/catalog/
  5. Write manifest.json: old_supabase_url → https://yourdomain.ru/catalog/...
...
### Connectivity (from Russian IP)

- [ ] `https://yourdomain.ru` loads without VPN
- [ ] `https://yourdomain.ru/catalog/...` — images load
...
| 3 Timeweb VPS | After domain registered (or in parallel) |
| 4 Deploy + migrate | After VPS ready; scripts can be written during step 1 |
| 6 Metrics/APIs | After app deploys on VPS |
...
flowchart TD
  ...
  S2[Step 2 Domain] --> S4
  S3[Step 3 Timeweb VPS] --> S4[Step 4 Deploy + migrate]
  S5script --> S4
...
- No dual-write between Supabase and VPS Postgres — **one-time export**, then independent ops
