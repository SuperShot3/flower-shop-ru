/**
 * Build tel: / wa.me links from order phone fields (Russia default CC 7).
 */
...
export function e164Digits(phone: string | null | undefined, countryCode: string | null | undefined): string | null {
  ...
  const cc = (countryCode?.replace(/\D/g, '') || '7') || '7';
  if (digits.startsWith(cc)) return digits;
  if (cc === '7' && digits.startsWith('8') && digits.length === 11) {
    return '7' + digits.slice(1);
  }
  if (digits.startsWith('0')) return cc + digits.slice(1);
  if (cc === '7' && digits.length === 10) return cc + digits;
